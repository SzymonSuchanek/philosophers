...
/*   Updated: 2024/08/18 23:00:42 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    init_philo(int total_philos)
{
    pthread_t philo[total_philos];
    int i;
...
// start routine
// if 2 forks ((n && n + 1) arent under mutex, put a lock on them and let the thread start using them for time_to_eat miliseconds
...
// put n thread to sleep for time_to_sleep milisecond, printing a message  "timestamp_in_ms n is sleeping"
// after time_to_sleep miliseconds put thread on thinking mode printing "timestamp_in_ms n is thinking", 
// after that mode is over, n philosopher has chance to take 2 forks again to start eating, and it needs to be before the timer goes to 0 
...
// otherwise simulation stops, sending a message "timestamp_in_ms n died" not later than 10 miliseconds after the death.
// last argument might indicate number of cycles for each thread, if we reach the number of cycles for specific thread, we stop the thread from executing,